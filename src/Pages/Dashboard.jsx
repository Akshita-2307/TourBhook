...
function AllUsersModal({ users, onClose, onRoleUpdate }) {
            ...
            >
                <div className="blocked-modal-header">
                    <div>
                        ...
                        <h2 id="all-users-title">All Registered Users ({userList.length})</h2>
                    </div>
                    <button type="button" onClick={onClose} aria-label="Close users list">
                        ×
                    </button>
                </div>
                {userList.length === 0 ? (
                    <div className="moderation-empty-state">
                        ...
                        <span>Registered database accounts will appear here.</span>
                    </div>
                ) : (
                    <div className="blocked-users-list">
                        {userList.map((user, index) => (
                            <article key={user.id || index} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                                <span className="blocked-user-rank">{String(index + 1).padStart(2, "0")}</span>
                                <span className="avatar avatar-sidebar">{user.email ? user.email.charAt(0).toUpperCase() : "U"}</span>
                                <div className="blocked-user-identity" style={{ flex: 1 }}>
                                    <strong>{user.email}</strong>
                                    <span style={{ fontSize: "12px", color: "#64748b" }}>KYC: {user.kyc_status || "PENDING"}</span>
                                </div>
                                <div>
                                    <label style={{ fontSize: "11px", display: "block", marginBottom: "2px" }}>Assign RBAC Role</label>
                                    <select
...
function Dashboard({ data: initialMockData, onLogout }) {
    const [activePage, setActivePage] = useState("overview");
    const [selectedChartPeriod, setSelectedChartPeriod] = useState("week");
    const [liveData, setLiveData] = useState(initialMockData);
                ...
                if (Array.isArray(usersList) && usersList.length > 0) {
                    ...
                    setLiveData((current) => ({
                        ...
                        metrics: current.metrics.map((metric) => {
                            if (metric.id === "total-users" || metric.label?.includes("Users")) {
                                return { ...metric, value: usersList.length };
                            }
                            return metric;
                        }),
                        preferredCities: usersList.slice(0, 5).map((user, index) => ({
                            id: user.id || index,
                            name: user.email ? user.email.split("@")[0] : `User ${index + 1}`,
                            accent: index % 2 === 0 ? "indigo" : "cyan",
                            symbol: user.email ? user.email.charAt(0).toUpperCase() : "U",
                            users: 100 + index * 45,
                            sharePercent: Math.max(10, 45 - index * 8),
                        })),
                    }));
                }
            })
            .catch((err) => {
                console.error("Could not load backend users, falling back to mock data:", err);
            });
    };
    ...
    const highestShare = Math.max(
        ...liveData.preferredCities.map((city) => city.sharePercent),
    );
    const selectedChart = liveData.newUsersChart.periods[selectedChartPeriod];
    ...
    return (
        <div className="dashboard-shell">
            <aside className="sidebar">
                ...
                <div className="sidebar-lower">
                    <div className="sidebar-user">
                        <span className="avatar avatar-sidebar">{liveData.user.initials}</span>
                        <div>
                            <strong>{liveData.user.fullName}</strong>
                            <span>{liveData.user.role}</span>
                        </div>
            ...
            <div className="dashboard-workspace">
                <header className="topbar">
                    ...
                    <div className="topbar-profile">
                        <span className="avatar">{liveData.user.initials}</span>
                        <button
                    ...
                    <main className="dashboard-content">
                        <section className="dashboard-heading">
                            <div>
                                <h1>User Activity</h1>
                                <p>
                                    Welcome back, {liveData.user.firstName}! Here&apos;s what&apos;s happening on
                                    TourBhook today.
                                </p>
                            </div>
                            <DateRangeControl initialRange={liveData.reportingPeriod} />
                        </section>
                        ...
                        <section className="metrics-grid" aria-label="Activity summary">
                            {liveData.metrics.map((metric) => (
                                <MetricCard
                                    metric={metric}
                                    comparisonLabel={liveData.reportingPeriod.comparisonLabel}
                                    key={metric.id}
                                />
                            ))}
                        </section>
                        ...
                        <section className="dashboard-panel chart-panel">
                            <div className="panel-heading">
                                <h2>{liveData.newUsersChart.title}</h2>
                                <ChartPeriodControl
                                    periods={liveData.newUsersChart.periods}
                        ...
                        <section className="dashboard-panel city-panel">
                            <div className="panel-heading">
                                <h2>Registered Database Users</h2>
                                <button
                                    className="plain-button view-all"
                                    type="button"
                            ...
                            <div className="city-table-scroll">
                                <table className="city-table">
                                    ...
                                    <tbody>
                                    {liveData.preferredCities.map((city, index) => (
                                        <tr key={city.id}>
            ...
            {showAllUsersModal && (
                <AllUsersModal users={allUsers} onClose={() => setShowAllUsersModal(false)} onRoleUpdate={fetchUsers} />
            )}
        </div>
    );
}
