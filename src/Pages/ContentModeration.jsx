...
function SeverityBadge({ severity }) {
  ...
  return (
      <span className={`moderation-badge severity-${(severity || "low").toLowerCase()}`}>
...
function StatusBadge({ status }) {
  return (
      <span className={`moderation-badge status-${(status || "pending").toLowerCase()}`}>
  ...
  const cards = [
    {
      ...
      icon: "reports",
      tone: "blue",
      detail: `↓ ${summary?.openReportsChange || 0}%`,
    ...
    },
    {
      ...
      icon: "alert",
      tone: "red",
      detail: `↑ ${summary?.criticalReportsToday || 0} today`,
                    ...
                    <tr key={report.id}>
                      ...
                      <td>
                        <div className="report-content-cell">
                          {report.contentImage && <img src={report.contentImage} alt="" />}
      ...
      <aside className="report-drawer" aria-label={`Review ${report.id}`}>
        ...
        <section className="drawer-section">
          ...
          <div className={`drawer-content-preview${report.contentRemoved ? " is-removed" : ""}`}>
            {report.contentImage && <img src={report.contentImage} alt="Reported travel content" />}
            <div>
              <span>{report.contentRemoved ? "Content removed" : report.contentText}</span>
              <small>
                {formatRelativeTime(report.submittedAt, referenceTime)} · {report.reportCount || 1} reports
              </small>
            </div>
          </div>
        </section>
        ...
        <section className="drawer-section report-information">
          ...
          <div>
            ...
            <strong>{report.contentId || "N/A"}</strong>
          </div>
        </section>
        ...
        >
          <small>Previous reports: {report.reporter?.previousReports || 0}</small>
        </PersonDetails>
        ...
        >
          <small>
            Account status: <b>{report.reportedUser?.accountStatus || "Active"}</b>
          </small>
          <small>Previous violations: {report.reportedUser?.previousReports || 0}</small>
        </PersonDetails>
...
function ContentModeration() {
  ...
  // Directly override with fetched backend records
  useEffect(() => {
    fetch("http://localhost:5000/api/moderation-reports")
        .then((res) => res.json())
        .then((data) => {
        ...
        .catch((err) => {
          console.warn("Backend offline, using hook fallback:", err);
          setIsLoading(false);
        });
  }, []);

  const displayReports = liveReports.length > 0 ? liveReports : moderation.visibleReports;
  const totalCount = liveReports.length > 0 ? liveReports.length : moderation.filteredReports.length;

  const dynamicSummary = {
    ...moderation.summary,
    openReports: liveReports.length > 0 ? liveReports.length : moderation.summary.openReports,
    criticalReports: liveReports.length > 0 ? liveReports.filter(r => r.severity === "High" || r.severity === "Severe").length : moderation.summary.criticalReports
  };

  const selectedReport = displayReports.find(
