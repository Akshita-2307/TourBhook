...
function StatusBadge({ status }) {
  return <span className={`kyc-status kyc-status-${status.toLowerCase()}`}>{status}</span>;
}
              ...
              {submissions.map((submission, index) => {
                ...
                return (
                  <tr className={selectedId === submission.id ? "is-selected" : ""} key={submission.id}>
                    <td>{index + 1}</td>
                    <td><div className="kyc-partner-cell"><span className="kyc-partner-thumb" style={{ "--partner-color": submission.color }}>{submission.initials}</span><span><strong>{submission.partnerName}</strong><small>{submission.email}</small></span></div></td>
                    <td>{submission.id}</td>
  ...
  return (
    <aside className="kyc-review-panel" aria-label={`Review ${submission.id}`}>
      ...
      <section className="kyc-applicant">
        <span className="kyc-applicant-photo" style={{ "--partner-color": submission.color }}>{submission.initials}</span>
      ...
      <section className="kyc-review-documents">
        <div className="kyc-document-list"><h3>Submitted Documents ({documentCount}/4)</h3>{kycDocumentDefinitions.map((definition) => {
          const document = submission.documents[definition.key];
